...
import numpy as np
def init():
    if not os.path.exists('./spider/navData.csv'):
        with open('./spider/navData.csv','w',encoding='utf8',newline='') as csvfile:
...
def wirterRow(row):
        with open('./spider/navData.csv','a',encoding='utf8',newline='') as csvfile:
            wirter = csv.writer(csvfile)
            wirter.writerow(row)
            print("写入成功")
